...
docker exec \
  ...
  peer chaincode instantiate \
    ...
    -v 1.0 \
    -c '{"Args":[]}' \
    -P "OutOf(1,'Org1MSP.member','Org2MSP.member','Org3MSP.member')" \
    --tls \
...
echo "The transaction is sent to the three peers with the chaincode installed (peer0.org1.example.com, peer0.org2.example.com, peer0.org3.example.com) so that chaincode is built before receiving the following requests"
docker exec \
  -e CORE_PEER_LOCALMSPID=Org1MSP \
  -e CORE_PEER_MSPCONFIGPATH=${ORG1_MSPCONFIGPATH} \
  cli \
