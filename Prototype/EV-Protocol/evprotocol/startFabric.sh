...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang"  ]; then
	...
	CC_SRC_PATH=github.com/chaincode/evprotocol/go
elif [ "$CC_SRC_LANGUAGE" = "javascript" ]; then
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH=/opt/gopath/src/github.com/chaincode/evprotocol/javascript
else
	echo The chaincode language ${CC_SRC_LANGUAGE} is not supported by this script
	echo Supported chaincode languages are: go and javascript
	exit 1
fi
...
Next, use the EVProtocol applications to interact with the deployed EVProtocol contract.
...
JavaScript:
  ...
  Next step is to enroll the admin user, and register a new user which will be used by
  the application to interact with the deployed EVProtocol contract:
    node enrollAdmin
    node registerUser
  ...
  return all cars in the region defined in the request, but you can update the application
  to evaluate other transactions:
    node query
