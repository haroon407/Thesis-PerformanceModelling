...
echo "The transaction is sent to the four peers with the chaincode installed (peer0.org1.example.com, peer0.org2.example.com, peer0.org3.example.com, peer0.org4.example.com) so that chaincode is built before receiving the following requests"
docker exec \
  -e CORE_PEER_LOCALMSPID=Org1MSP \
  -e CORE_PEER_MSPCONFIGPATH=${ORG1_MSPCONFIGPATH} \
  cli \
